...
class ManageTeamsViewModel(
    ...
    val players: StateFlow<List<Player>> = playerRepository.getPlayersFlow()
        .stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = emptyList()
    ...
    fun updateTeamRoster(teamId: String, playerIds: List<String>) {
        viewModelScope.launch {
            teamRepository.updateTeamRoster(teamId, playerIds)
        }
    }
}
